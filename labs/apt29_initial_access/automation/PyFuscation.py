...
def findCustomParams(iFile, VARs):
    PARAMs = {}
    READ = False
    start = 0
    end = 0
...
# Changed Function name replacement to also use random strings to match CTI more closely
def findFUNCs(iFile):
        ...
        for line in f:
            ...
            if funcMatch and funcMatch.group(1) not in FUNCs: 
                if funcMatch.group(1) == "main":
                    continue
                vNum = 9999
                new = ''.join([random.choice(string.ascii_letters) for n in range(8)]) #randomString(wordList)
